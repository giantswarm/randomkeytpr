...
// SearchKeys watches for keys secrets of a cluster and returns it as
// assets bundle.
func (s *Service) SearchKeys(clusterID string) (AssetsBundle, error) {
	assetsBundle := make(AssetsBundle)

	for _, componentName := range ClusterComponents {
		ab, err := s.SearchKeysForComponent(clusterID, componentName.String())
		if err != nil {
...
// SearchKeysForComponent watches for keys secrets of a single cluster component and
// returns it as assets bundle.
func (s *Service) SearchKeysForComponent(clusterID, componentName string) (AssetsBundle, error) {
	...
	for {
		...
		case event, ok := <-watcher.ResultChan():
			...
			case watch.Added:
				...
				for _, assetType := range RandomKeyAssetTypes {
